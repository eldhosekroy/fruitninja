...
import os
import re
# --- TensorFlow GPU / quality & perf setup (paste near top, after imports) ---
import os
# reduce verbose TF logs if you want
os.environ.setdefault("TF_CPP_MIN_LOG_LEVEL", "2")  # 0=all, 1=warnings, 2=errors, 3=fatal

# Optionally force which GPU(s) TF can see (uncomment and set index list, e.g. "0" or "0,1")
# os.environ["CUDA_VISIBLE_DEVICES"] = "0"

import tensorflow as tf

# Choose a "quality" policy:
# - "high_quality" uses pure float32 (best numeric precision, slower)
# - "mixed_precision" uses float16 compute where possible (faster on modern GPUs)
QUALITY_MODE = "mixed_precision"  # or "high_quality"

# Print TensorFlow version (helpful for debugging)
print(f"TensorFlow {tf.__version__} initializing...")
...
# Performance: enable XLA JIT (often improves compute kernels)
try:
    tf.config.optimizer.set_jit(True)
    print("XLA JIT enabled.")
except Exception:
    print("Could not enable XLA JIT (OK).")

# Threading / parallelism (tweak if needed)
...
try:
    ...
    if QUALITY_MODE == "mixed_precision":
        mixed_precision.set_global_policy("mixed_float16")
        print("Using mixed_float16 policy (faster on GPU).")
    else:
        mixed_precision.set_global_policy("float32")
        print("Using float32 policy (higher numeric precision).")
except Exception as e:
    print("Could not set mixed precision policy:", e)

# optional: check TF can access CUDA (runtime check)
try:
    gpu_ok = tf.test.is_built_with_cuda()
    print("TF built with CUDA support:", gpu_ok)
    print("Physical GPU available according to TF:", tf.config.list_physical_devices('GPU') != [])
except Exception as e:
    print("GPU runtime check failed:", e)
# --------------------------------------------------------------------------
...
CAM_SRC = "http://10.140.217.200:8080/video"
FRAME_RATE = 30
SPAWN_INTERVAL = 0.9
COMBO_WINDOW = 1.4
